...
def main(in_files, ref_file, out_file, write_probs=True):
    ...
    # x y z class wavefrorms
    length = 60*5
    xyzwc = np.zeros(length+3)
    xyzwc_arr = np.zeros((1, length+3))
        ...
        with open(path) as f:
            ...
            for s_line in f:
                ...
                for i in range(len(line)):
                    #格納
                    if(i<3):xyzwc[i] = line[i]
                    if(i==3 or i==4):continue
                    if(i>4):xyzwc[i-1] = line[i]
                # print (xyzwc)
                # search ref x y z class
                multiples = tree.query_ball_point(xyzwc[0:2], r=0.01, eps=0.0001) ## xy
                if(len(multiples)>0):
                    # print(multiples)
                    xyzwc[3] = out_labels[multiples[0]]
                    # print(xyzwc)
                    xyzwc_arrs.append(xyzwc.tolist())

                    count += 1

        # save data
        print(np.array(xyzwc_arrs).shape)
        np.savetxt(path.replace(".txt","_class.txt"),np.array(xyzwc_arrs) )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--outFile', required=False, help='path to write output to')
    parser.add_argument('--writeProbs', default=False, type=bool, help='write class probabilities')
